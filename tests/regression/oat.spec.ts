...
test.describe('Partner Management flows', () => {
  ...
  test('Users Tab', async ({ page }, testInfo) => {
    ...
    await gotoWithRetry(page, '/dashboard/users');
    await expect(page.locator('body')).toBeVisible();
  ...
  test('Hierarchy & Permissions Tab', async ({ page }, testInfo) => {
    ...
    await gotoWithRetry(page, '/dashboard/hierarchies/hierarchies');
    await expect(page.locator('body')).toBeVisible();
    // await expect(page).toHaveURL(/hierarchyAndPermission/i);
    await assertSessionActive(page);
  });

  test('Hierarchy Domains Tab — permissions', async ({ page }, testInfo) => {
    addDescription(testInfo, 'Direct navigation to hierarchy domains (permissions) should render and keep the expected URL.');
    await gotoWithRetry(page, '/dashboard/hierarchies/permissions');
    await expect(page.locator('body')).toBeVisible();
    // await expect(page).toHaveURL(/hierarchyDomains/i);
    await assertSessionActive(page);
  });

  test('Hierarchy Domains Tab — with-domain', async ({ page }, testInfo) => {
    addDescription(testInfo, 'Direct navigation to hierarchy domains (with-domain) should render and keep the expected URL.');
    await gotoWithRetry(page, '/dashboard/hierarchies/with-domain');
    await expect(page.locator('body')).toBeVisible();
    // await expect(page).toHaveURL(/hierarchyDomains/i);
    await assertSessionActive(page);
  });
